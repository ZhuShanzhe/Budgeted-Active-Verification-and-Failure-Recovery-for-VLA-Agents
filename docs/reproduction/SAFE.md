...
服务器直连 Google Drive 会超时，`/etc/network_turbo` 代理访问 Google 下载域名会返回 503。仓库提供 `notebooks/colab_stream_safe_rollouts.ipynb`：在 Google Colab 中运行后，数据会由 Google Drive 经 Colab 内存直接流式写入服务器 SFTP，不经过用户电脑，也不占用本地下载流量。Notebook 固定校验服务器 ED25519 指纹，密码使用隐藏输入且不写入文件。
